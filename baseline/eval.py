...
EVAL_TASKS = [
    ...
    # Derived from Zone 1 CSV_FIELD_GROUPS metadata, not from actual Neo4j values.
    {
        ...
        "cypher": "MATCH (c:Entity {entity_type: 'ClaimRecord'}) RETURN c.id AS id, c.entity_type AS type LIMIT 3",
        "keywords": ["CLM-"],
    },
    {
        "id": 22,
        ...
        "cypher": "MATCH (p:Entity {entity_type: 'PolicyRecord'}) RETURN p.id AS id, p.entity_type AS type LIMIT 3",
        "keywords": ["POL-"],
    },
    {
        "id": 23,
        ...
        "cypher": "MATCH (c:Entity {entity_type: 'ClaimRecord'})-[r:HAS_AMOUNT_PAID_ON_BUILDING_CLAIM]->(v) RETURN c.id AS claim, v.id AS amount LIMIT 5",
        "keywords": ["CLM-"],
    },
    {
        "id": 24,
        ...
        "cypher": "MATCH (c:Entity {entity_type: 'ClaimRecord'})-[r:HAS_DATE_OF_LOSS]->(v) RETURN c.id AS claim, v.id AS date_of_loss LIMIT 5",
        "keywords": ["CLM-"],
    },
    {
        "id": 25,
        ...
        "cypher": "MATCH (p:Entity {entity_type: 'PolicyRecord'})-[r:HAS_TOTAL_BUILDING_INSURANCE_COVERAGE]->(v) RETURN p.id AS policy, v.id AS coverage_amount LIMIT 5",
        "keywords": ["POL-"],
    },
    {
        "id": 26,
        ...
        "cypher": "MATCH (p:Entity {entity_type: 'PolicyRecord'})-[r:HAS_RATED_FLOOD_ZONE]->(v) RETURN p.id AS policy, v.id AS flood_zone LIMIT 5",
        "keywords": ["POL-"],
    },
    {
        "id": 27,
        ...
        "cypher": "MATCH (a)-[r:LINKED_TO]->(b) WHERE a.entity_type IN ['PolicyRecord','ClaimRecord'] AND b.entity_type IN ['PolicyRecord','ClaimRecord'] RETURN a.id AS from_id, a.entity_type AS from_type, b.id AS to_id, b.entity_type AS to_type LIMIT 10",
        "keywords": ["POL-", "CLM-"],
    },
    {
        "id": 28,
        ...
        "cypher": "MATCH (rec)-[:HAS_RATED_FLOOD_ZONE]->(z)<-[r]-(concept) WHERE rec.entity_type IN ['PolicyRecord','ClaimRecord'] AND concept.source_type IS NULL RETURN z.id AS zone, type(r) AS rel, concept.id AS concept LIMIT 10",
        "keywords": ["zone"],
    },
    {
        "id": 29,
        ...
        "cypher": "MATCH (c:Entity {entity_type: 'ClaimRecord'})-[r:HAS_CAUSE_OF_DAMAGE]->(v) RETURN c.id AS claim, v.id AS cause LIMIT 5",
        "keywords": ["CLM-"],
    },
    {
        "id": 30,
